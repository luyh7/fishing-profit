...
仓库内还提供了 `.githooks/pre-commit`，首次 clone 后执行 `./setup.sh` 即可启用（或手动执行 `git config core.hooksPath .githooks`），之后每次提交前会自动递增 `gitCommitCount` 并重新暂存 `config.js`。功能类提交的 changelog 规则见 [`CHANGELOG_CONTRACT.md`](CHANGELOG_CONTRACT.md)。
