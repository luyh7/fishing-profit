...
- 用户要求分析游戏源码时，必须先在子模块中执行 `git fetch origin main`，以最新 `origin/main` 为目标 SHA；更新前必须记录旧 SHA 和目标 SHA，并先检查 `旧 SHA..目标 SHA` 的源码差异。除此之外，不得把跟随上游 `main` 作为日常更新步骤。
- 源码分析必须采用“增量审查 + 整体闭环复核”：先检查 `旧 SHA..目标 SHA` 的全部变更，再从开始钓鱼、模拟捕获、鱼饵消耗、天气与 Buff、停止结算、奖励入库到出售定价，完整复核实际收益链路；不得仅依据变更文件判断影响。无需无差别逐文件重读上游仓库，但必须确认所有影响利润的入口、配置和下游消费者均已覆盖。
- 分析时应同步检查利润网实现、测试和文档；验证完成后才可将子模块以 detached HEAD 固定到目标 SHA，并在主仓库提交新的 gitlink。
