...
test("两套完整 PMF 与当前 Python 评分器的精确基准一致", () => {
  const expectedHashes = {
    normal: "4af9cad85180adc73db7e2f889dc0052e725f42fe082b3602377192ada5fdaae",
    hengjiyuan:
      "275560c5bc0a55df9d16758a93077f4bbd06a91dbb9a3fd9405b6bcb9fe248c8",
  };
...
test("JavaScript 评分移植覆盖源码中的典型组合规则", () => {
  ...
  assert.equal(score("000111"), 13_374_720);
  assert.equal(score("000001"), 12_469_266);
  assert.equal(score("000000"), 15_047_072);
  assert.equal(score("777777"), 15_047_072);
});

test("普通域单候选的均值和奖池概率匹配精确基准", () => {
  const summary = summarize(distribution.domains.normal);

  assertClose(summary.meanRawScore, 0.8402781015560007);
  assertClose(summary.poolProbabilities.none, 0.489104);
  assertClose(summary.poolProbabilities.low, 0.39497000000000004);
  assertClose(summary.poolProbabilities.middle, 0.09953199999999995);
  assertClose(summary.poolProbabilities.high, 0.015550000000000064);
  assertClose(summary.poolProbabilities.ultimate, 0.0008439999999999559);
});

test("恒纪元域单候选的均值和奖池概率匹配精确基准", () => {
  const summary = summarize(distribution.domains.hengjiyuan);

  assertClose(summary.meanRawScore, 1.5149011100476832);
  assertClose(summary.poolProbabilities.none, 0.30218701391427044);
  assertClose(summary.poolProbabilities.low, 0.4656903161097841);
  assertClose(summary.poolProbabilities.middle, 0.18320597710137787);
  assertClose(summary.poolProbabilities.high, 0.04516825472379704);
  assertClose(summary.poolProbabilities.ultimate, 0.0037484381507705278);
});

test("普通域双候选择优使用 PMF 的精确 CDF", () => {
  const summary = summarize(distribution.domains.normal, 2);

  assertClose(summary.meanRawScore, 1.4414374499158429);
  assertClose(summary.poolProbabilities.none, 0.239222722816);
  assertClose(summary.poolProbabilities.low, 0.54236411466);
  assertClose(summary.poolProbabilities.middle, 0.18589392575999997);
  assertClose(summary.poolProbabilities.high, 0.03083194910000009);
  assertClose(summary.poolProbabilities.ultimate, 0.001687287663999948);
});
